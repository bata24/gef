...
The installers support Ubuntu, Debian, and Arch Linux. They use `apt-get` on Debian-based systems and `pacman` on Arch Linux.
On Arch Linux, they run a full `pacman -Syu` upgrade because partial upgrades are not supported.
...
Make sure GDB is run by the same user who installed GEF.
The installer writes to that user's `$HOME/.gdbinit` and installs GEF under `$HOME/.gef` by default.
...
Most commands have been validated on kernels from 3.x up to 7.0.x. Newer kernels may work but are not fully validated.
