...
- Run the following command:
    ...
    ```
    - Notes
        - `gef.py` is installed to `/root/.gef/gef.py` for simplicity.
        - Required Python packages go into `/root/.gef/.venv-gef`.
        - `/root/.gef` is registered in `/root/.gdbinit`.
        - For other options (for non-`root` install, etc), see [docs/FAQ.md](docs/FAQ.md).
